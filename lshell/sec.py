...
def warn_count(messagetype, command, conf, strict=None, ssh=None):
    ...
    log = conf["logpath"]
    suffix = utils.log_ip_suffix(conf)
    if not ssh:
        if strict:
            conf["warning_counter"] -= 1
            if conf["warning_counter"] < 0:
                log.critical(
                    '*** forbidden %s -> "%s"%s' % (messagetype, command, suffix)
                )
                log.critical("*** Kicked out%s" % suffix)
                sys.exit(1)
            else:
                log.critical(
                    '*** forbidden %s -> "%s"%s' % (messagetype, command, suffix)
                ...
                log.error(
                    "*** User warned, counter: %s%s"
                    % (conf["warning_counter"], suffix)
                )
